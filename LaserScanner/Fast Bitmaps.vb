...
Public Class FastBitmap
    ...
    Public Sub New(W As Integer, H As Integer, fmt As PixelFormat)
        Realbmp = New Bitmap(W, H, fmt)
        format = fmt
        width = W
        height = H
        If fmt <> PixelFormat.Format8bppIndexed Then GR = Graphics.FromImage(Realbmp)

        Dim bmpData As BitmapData = Realbmp.LockBits(New Rectangle(0, 0, Realbmp.Width, Realbmp.Height), ImageLockMode.ReadWrite, Realbmp.PixelFormat)
        stride = bmpData.Stride
        Realbmp.UnlockBits(bmpData)
    ...
    Public Sub New(b As Bitmap)
        ...
        Realbmp = New Bitmap(width, height, format)
        GR = Graphics.FromImage(Realbmp)
    ...
    Public Sub MakeNewFromBytes()
        byteToBitmap(bytes, Realbmp)
        If Realbmp.PixelFormat <> PixelFormat.Format8bppIndexed Then GR = Graphics.FromImage(Realbmp)
    End Sub
    ...
    Public Sub MakeFromBytes(bytes As Byte())
        ...
        Dim rect As Rectangle = New Rectangle(0, 0, Realbmp.Width, Realbmp.Height)
        Dim bmpData As BitmapData = Realbmp.LockBits(rect, ImageLockMode.ReadWrite, Realbmp.PixelFormat)
        ' Copy the RGB values back to the bitmap
        Runtime.InteropServices.Marshal.Copy(bytes, 0, bmpData.Scan0, bmpData.Stride * Realbmp.Height)
        ' Unlock the bits.
        Realbmp.UnlockBits(bmpData)
